-- =============================================================================
--  Provisión de la BD de Vinzer dentro del Postgres del stack (contenedor
--  vinzer_postgres). Ejecutar como SUPERUSUARIO (usuario_pro).
--
--  NOTA: En el despliegue normal NO necesitas correr este archivo a mano — el
--  entrypoint del backend ejecuta `scripts/provision_db.sh`, que hace esto de
--  forma idempotente. Se deja como referencia / provisión manual.
--
--  Crea:
--    1) El rol de la APP `vinzer_app` — usuario NORMAL: el RLS se aplica sobre
--       él, garantizando el aislamiento multi-tenant. NO debe ser superusuario
--       ni tener BYPASSRLS.
--    2) La base de datos `v3_saas` propiedad del superusuario.
--
--  El superusuario `usuario_pro` se usa como DB_ADMIN para los respaldos
--  (ignora RLS por ser superusuario).
--
--  Uso (desde el host del VPS):
--    docker exec -i vinzer_postgres \
--      psql -U usuario_pro -d postgres -f - < setup_production_roles.sql
--  (o copiar el archivo al contenedor y pasar -f /ruta/al/archivo.sql)
-- =============================================================================

-- 1) Rol de la aplicación (cambia la contraseña por la real de tu .env) --------
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'vinzer_app') THEN
        CREATE ROLE vinzer_app LOGIN PASSWORD 'CAMBIA_ESTA_PASSWORD'
            NOSUPERUSER NOCREATEDB NOCREATEROLE NOBYPASSRLS;
    END IF;
END
$$;

-- 2) Base de datos -------------------------------------------------------------
-- CREATE DATABASE no puede ir dentro de un bloque DO/transacción; se ejecuta
-- de forma condicional con \gexec.
SELECT 'CREATE DATABASE v3_saas OWNER usuario_pro'
WHERE NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'v3_saas')
\gexec

-- 3) Privilegios del rol de la app sobre la base -------------------------------
GRANT CONNECT ON DATABASE v3_saas TO vinzer_app;

-- Los privilegios sobre el schema/tablas se otorgan DENTRO de la base v3_saas.
-- Reconéctate a ella y ejecuta el bloque siguiente:
--    \connect v3_saas
--    GRANT USAGE, CREATE ON SCHEMA public TO vinzer_app;
--    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO vinzer_app;
--    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO vinzer_app;
--    ALTER DEFAULT PRIVILEGES IN SCHEMA public
--        GRANT ALL ON TABLES TO vinzer_app;
--    ALTER DEFAULT PRIVILEGES IN SCHEMA public
--        GRANT ALL ON SEQUENCES TO vinzer_app;
