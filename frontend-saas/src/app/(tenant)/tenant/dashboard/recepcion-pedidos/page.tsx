...
import OrderSlideOver from '@/components/tenant/operations/OrderSlideOver';
...
export default function CremationsPage() {
    ...
    const handleNewService = () => {
        ...
        router.push('/dashboard/recepcion-pedidos/registro');
    };
    const handleOpen = (id: number) => router.push(`/dashboard/recepcion-pedidos/registro?id=${id}`);
    ...
    const KanbanCard = ({ item, nextStatus, nextLabel, icon: Icon, colorClass, glowColor, isSelected, onSelect }: any) => {
        ...
        const handleCopyCode = async (e: React.MouseEvent) => {
            e.stopPropagation();
            if (!item.verification_code) return;
            const success = await copyToClipboard(item.verification_code);
            if (success) {
                showToast('Código de tracking copiado', 'success');
            } else {
        ...
        return (
            ...
            >
                ...
                <div className="flex gap-3 relative z-10">
                    <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 overflow-hidden shrink-0 shadow-inner group-hover:border-primary/40 transition-colors">
                        {item.pet?.image_url ? (
                    ...
                    <div className="min-w-0 flex-1 pr-6">
                        ...
                        <p className="text-xs text-muted-foreground truncate">{item.pet?.customer?.name || 'Cliente no asignado'}</p>
                        {item.created_at && (
                            <p className="text-[10px] text-muted-foreground/60 mt-0.5 font-medium">
                                {new Date(item.created_at).toLocaleDateString('es-CL')} {new Date(item.created_at).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}
                            </p>
                        )}
                    </div>
                </div>

                {/* Badges row + Quick Tracking Share */}
                <div className="flex flex-wrap items-center justify-between gap-1.5 relative z-10 pt-1">
                    <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        {item.verification_code && (
                            <button
                                ...
                                className="text-[10px] font-mono font-bold bg-primary/10 hover:bg-primary/20 text-primary px-2 py-0.5 rounded-md border border-primary/20 transition flex items-center gap-1"
                    ...
                    {/* Botones rápidos de Compartir Tracking */}
                    {item.verification_code && (
                        <div className="flex items-center gap-1">
                            <button
                                ...
                                onClick={async (e) => {
                                    e.stopPropagation();
                                    const tenantSlug = tenantData?.slug || 'crematorio';
                                    const url = buildTrackingUrl(tenantSlug, item.pet?.name || 'mascota', item.verification_code!);
                                    if (await copyToClipboard(url)) {
                                        showToast('Enlace de tracking copiado al portapapeles', 'success');
                                    }
                                }}
                                className="p-1 rounded-md bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-white border border-white/5 transition flex items-center gap-1 text-[10px]"
                                title="Copiar link directo de seguimiento para la familia"
                            >
                                <Share2 size={11} className="text-primary" />
                                <span className="hidden sm:inline font-bold">Link</span>
                            </button>

                            {item.pet?.customer?.phone && (
                                <button
                                    type="button"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        ...
                                        const msg = encodeURIComponent(`Hola ${item.pet?.customer?.name || ''}, te compartimos el enlace para seguir el proceso de ${item.pet?.name || 'tu mascota'}: ${url}`);
                                        window.open(`https://wa.me/${cleanPhone}?text=${msg}`, '_blank');
                                    }}
                                    className="p-1 rounded-md bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] border border-[#25D366]/20 transition flex items-center gap-1 text-[10px]"
                                    title="Enviar seguimiento por WhatsApp a la familia"
                                >
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-white/5 relative z-10">
                    {nextStatus && (
                        <button
                            ...
                            className={`flex-1 py-2 px-3 rounded-xl border border-transparent text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all ${colorClass}`}
                        >
                            {nextLabel}
                            <Icon size={14} />
                        </button>
                    ...
                    {isCompleted && (
                        <Link
                            ...
                            className="flex-1 py-2 px-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 text-[11px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition"
                        >
                            <FileText size={14} />
                            Certificado
                        </Link>
            ...
            <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
                <div>
                    ...
                    <div className="flex items-center gap-4">
                        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-white">Recepción y Pedidos<span className="text-primary ml-1">.</span></h1>
                    </div>
                    <p className="text-muted-foreground mt-1.5 text-sm max-w-xl">
                        Gestiona el ingreso y flujo de servicios. Usa el Tablero Kanban para control rápido o la Lista para detalles.
                    </p>
                ...
                <div className="space-y-3">
                    {filteredCremations.map((item: Cremation) => (
                        <OperationRow
                            ...
                            onOpen={(id) => {
                                const found = cremations.find(c => c.id === id);
                                if (found) {
