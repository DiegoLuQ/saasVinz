/**
 * Helpers para construir el código embebible del Widget de Vinzer.
...
export function buildStandaloneTemplate(key: string, base = getWidgetBaseUrl()): string {
    return `<!-- ====== Widget Vinzer (Productos, Servicios y Seguimiento) ====== -->
<div id="vinzer-widget-root"></div>

<style>
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@500;600;700;800&display=swap');
.vinzer-w{--vc:#19B5FE;--vd:#E0B84D;font-family:'Plus Jakarta Sans',system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:#0f172a;line-height:1.5;max-width:1120px;margin:0 auto;-webkit-font-smoothing:antialiased}
...
.vinzer-card{position:relative;border:1px solid #e8edf3;border-radius:18px;background:#fff;padding:18px;display:flex;flex-direction:column;gap:10px;box-shadow:0 1px 2px rgba(15,23,42,.04);transition:transform .25s,box-shadow .25s,border-color .25s;overflow:hidden}
.vinzer-card:hover{transform:translateY(-4px);box-shadow:0 18px 40px -12px rgba(15,23,42,.18);border-color:rgba(25,181,254,.45)}
.vinzer-thumb{width:100%;height:148px;border-radius:13px;overflow:hidden;background:linear-gradient(135deg,#f1f5f9,#e2e8f0)}
.vinzer-thumb img{width:100%;height:100%;object-fit:cover;display:block}
...
.vinzer-foot{display:flex;align-items:center;justify-content:space-between;margin-top:4px;padding-top:12px;border-top:1px solid #f1f5f9}
...
.vinzer-track:after{content:'';position:absolute;width:280px;height:280px;right:-90px;top:-120px;border-radius:50%;background:radial-gradient(circle,rgba(25,181,254,.25),transparent 70%)}
.vinzer-eyebrow{display:inline-flex;align-items:center;gap:7px;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.12em;color:var(--vd);margin-bottom:10px}
...
.vinzer-row input{width:100%;height:52px;padding:0 14px 0 42px;border:1px solid rgba(255,255,255,.14);border-radius:14px;font-size:15px;background:rgba(255,255,255,.06);color:#fff;font-family:inherit;font-weight:600;outline:none;transition:border-color .2s,box-shadow .2s}
.vinzer-row input::placeholder{color:#64748b;font-weight:500}
.vinzer-row input:focus{border-color:var(--vc);box-shadow:0 0 0 4px rgba(25,181,254,.18)}
.vinzer-row button{height:52px;padding:0 24px;border:0;border-radius:14px;background:linear-gradient(90deg,var(--vc),var(--vd));color:#020210;font-weight:800;font-size:14px;font-family:inherit;cursor:pointer;display:inline-flex;align-items:center;gap:8px}
...
.vinzer-ic{width:42px;height:42px;border-radius:12px;flex:none;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--vc),var(--vd));color:#020210}
.vinzer-st-label{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#94a3b8}
.vinzer-st-val{font-size:16px;font-weight:800;color:#fff}
.vinzer-st-stage{font-size:13px;color:var(--vc);font-weight:700}
.vinzer-msg{font-size:14px;color:#cbd5e1;padding:4px 2px}.vinzer-msg.err{color:#fca5a5}
.vinzer-empty,.vinzer-loading{font-size:14px;color:#94a3b8;padding:10px 0}
.vinzer-error{font-size:14px;color:#b91c1c;background:#fef2f2;border:1px solid #fecaca;border-radius:14px;padding:14px 16px}
...
(function(){
  ...
  function card(it,opts){opts=opts||{};var im=opts.withImage?'<div class="vinzer-thumb">'+(it.imagen?'<img src="'+esc(it.imagen)+'" alt="'+esc(it.nombre)+'" loading="lazy">':'')+'</div>':"";var tag=opts.isPlan?'<span class="vinzer-tag">Plan</span>':"";var ch="";if(it.servicios&&it.servicios.length){ch='<div class="vinzer-chips">'+it.servicios.slice(0,4).map(function(s){return '<span class="vinzer-chip">'+esc(s)+'</span>';}).join("")+'</div>';}return '<div class="vinzer-card'+(opts.isPlan?' vinzer-plan':'')+'">'+im+tag+'<h4>'+esc(it.nombre)+'</h4>'+(it.descripcion?'<p>'+esc(it.descripcion)+'</p>':'<p></p>')+ch+'<div class="vinzer-foot"><span class="vinzer-price"><small>Desde</small>'+clp(it.precio)+'</span></div></div>';}
  function group(t,ic,arr,opts){if(!arr||!arr.length)return "";return '<div class="vinzer-group"><div class="vinzer-head"><span class="vinzer-dot">'+ic+'</span><h3>'+esc(t)+'</h3><span class="vinzer-count">'+arr.length+'</span></div><div class="vinzer-grid">'+arr.map(function(i){return card(i,opts);}).join("")+'</div></div>';}
  function init(){
    var root=document.getElementById("vinzer-widget-root");if(!root)return;
    root.className="vinzer-w";
    root.innerHTML='<div class="vinzer-products"><p class="vinzer-loading">Cargando servicios...</p></div>'+
      '<div class="vinzer-track-wrap"><div class="vinzer-track"><span class="vinzer-eyebrow">'+I_PAW+' Seguimiento en tiempo real</span>'+
      '<h4>Sigue el proceso de tu mascota</h4><p>Ingresa el codigo que te entrego el crematorio para ver el estado del servicio.</p>'+
      '<div class="vinzer-row"><div class="vinzer-field">'+I_SEARCH+'<input class="vinzer-code" type="text" placeholder="Ej: SMROE2STJ4" autocomplete="off"></div><button class="vinzer-btn" type="button">'+I_SEARCH+' Buscar</button></div>'+
      '<div class="vinzer-result"></div></div></div>'+
      '<div class="vinzer-brand">'+I_PAW+' Powered by <b>Vinzer</b></div>';
    var prod=root.querySelector(".vinzer-products");
    fetch(CFG.api+"/api/public/widget/products-services?api_key="+encodeURIComponent(CFG.key))
      .then(function(r){if(!r.ok)throw 0;return r.json();})
      .then(function(d){var has=(d.plans&&d.plans.length)||(d.services&&d.services.length)||(d.products&&d.products.length);prod.innerHTML=has?(group("Planes",I_STAR,d.plans,{withImage:true,isPlan:true})+group("Servicios",I_TAG,d.services,{})+group("Productos",I_BOX,d.products,{withImage:true})):'<p class="vinzer-empty">No hay servicios disponibles.</p>';})
      .catch(function(){prod.innerHTML='<div class="vinzer-error">No se pudieron cargar los servicios.</div>';});
    var input=root.querySelector(".vinzer-code"),btn=root.querySelector(".vinzer-btn"),res=root.querySelector(".vinzer-result");
    function go(){var c=(input.value||"").trim();if(!c){res.innerHTML='<div class="vinzer-msg">Ingresa tu codigo de seguimiento.</div>';return;}btn.disabled=true;res.innerHTML='<div class="vinzer-msg">Buscando...</div>';
      fetch(CFG.api+"/api/public/widget/tracking/"+encodeURIComponent(c)+"?api_key="+encodeURIComponent(CFG.key))
        .then(function(r){if(!r.ok)throw 0;return r.json();})
        .then(function(d){res.innerHTML='<div class="vinzer-status"><span class="vinzer-ic">'+I_CHECK+'</span><div><div class="vinzer-st-label">Estado del servicio</div><div class="vinzer-st-val">'+esc(d.status||"En proceso")+'</div>'+(d.etapa?'<div class="vinzer-st-stage">Etapa: '+esc(d.etapa)+'</div>':'')+'</div></div>';})
        .catch(function(){res.innerHTML='<div class="vinzer-msg err">No encontramos un seguimiento con ese codigo.</div>';})
