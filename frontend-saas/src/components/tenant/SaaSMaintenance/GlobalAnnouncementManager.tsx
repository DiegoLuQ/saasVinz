...
export default function GlobalAnnouncementManager() {
    ...
    const fetchAnnouncements = async () => {
        setIsLoading(true);
        try {
            const data = await apiRequest(API_BASE_PATH);
            setAnnouncements(data || []);
        } catch (err: any) {
            console.error("Critical error in fetchAnnouncements:", err);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const fetchPlans = async () => {
        try {
            const data = await apiRequest('/api/internal/creator/subscription-plans');
