...
import { Activity, ShieldCheck, Database, Server, RefreshCw, HelpCircle, Cpu } from 'lucide-react';
...
export default function SystemHealth() {
    ...
    const checkHealth = async () => {
        ...
        try {
            ...
            const data = apiOk ? await apiResp.json() : null;

            setStatus({
                api: data?.api || (apiOk ? 'active' : 'error'),
                database: data?.database || (apiOk ? 'active' : 'warn'),
                storage: 'active',
                lastChecked: new Date()
            });
        } catch (err) {
    ...
    const handleOptimize = async () => {
        ...
        try {
            await apiRequest('/api/internal/health/optimize', {
                method: 'POST'
            });
            setOptimizeSuccess(true);
            // Refresh health check to get updated stats/status
            await checkHealth();
            // Clear success status after 4 seconds
            setTimeout(() => setOptimizeSuccess(false), 4000);
        } catch (err) {
            console.error('Error executing optimize:', err);
            alert('Error al optimizar la base de datos: ' + (err as any).message);
        } finally {
            setOptimizing(false);
        }
    };
        ...
        <div className="bg-[#0a192f] border border-white/10 rounded-3xl p-6 space-y-6 shadow-2xl overflow-hidden relative">
            ...
            <div className="pt-2 flex flex-col gap-3">
                <button
                    ...
                    disabled={optimizing}
                    className={`w-full py-3 px-4 rounded-2xl text-xs font-black transition-all flex items-center justify-center gap-2 border ${
                        optimizing
                            ? 'bg-white/5 border-white/10 text-white/40 cursor-not-allowed'
                            : optimizeSuccess
                            ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400'
                            : 'bg-primary/20 hover:bg-primary border-primary/25 text-white hover:shadow-lg hover:shadow-primary/10'
                    }`}
                >
                    {optimizing ? (
                        <>
